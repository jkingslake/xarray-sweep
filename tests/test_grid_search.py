...
def test_grid_search_with_scalar_output():
    def fn(a: int, b: int) -> int:
        return a + b

    out = grid_search(fn, show_progress=False, a=[1, 2], b=[10, 20])
...
def test_grid_search_with_dataset_output():
    def fn(a: int, b: int) -> xr.Dataset:
        return xr.Dataset({"value": xr.DataArray(a * b)})

    out = grid_search(fn, show_progress=False, a=[2, 3], b=[4, 5])
...
def test_empty_param_sweep_raises_value_error():
    ...
    try:
        grid_search(fn, show_progress=False, a=[])
